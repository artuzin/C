...
void DFS_Ponderada(Grafo* grafo, int vertice_inicial, int vertice_final) {
    ...
    Pilha* pilha = criar_pilha();   // Cria a pilha para a DFS
    // Vetor para armazenar a ordem de visitação
    int* ordem  = (int*) malloc(grafo->num_vertices * sizeof(int)); 
    int idx     = 0; // Índice para a ordem de visitação

    printf(BLUE_TEXT "\nBusca em Profundidade (DFS) a partir do vértice %d:\n" RESET_TEXT, vertice_inicial);

    push(pilha, vertice_inicial);   // Adiciona o vértice inicial à pilha
    imprime_pilha(pilha);           // Imprime o estado da pilha
    ...
    while (!pilha_vazia(pilha)) {
        vertice_atual = pop(pilha);     // Remove o vértice do topo da pilha
        ...
        imprime_pilha(pilha); // Imprime o estado da pilha após a remoção do vértice atual
    ...
    // Verifica se o vértice final foi encontrado e imprime o resultado
    if (encontrado) {
        imprimir_caminho(vertice_inicial, vertice_final, predecessor); // Imprime o caminho encontrado
    } else {
